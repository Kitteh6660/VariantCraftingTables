...
{
	@Nullable
	private final CraftingTableType craftingTableType;
	...
	private static final IDispenseItemBehavior DISPENSE_ITEM_BEHAVIOR = new DefaultDispenseItemBehavior() {
		...
		public ItemStack execute(IBlockSource source, ItemStack stack) {
			...
			if (stack.getItem() instanceof CraftingTableMinecartItem) {
				...
				minecartentity = new CraftingTableMinecartEntity(world, d0, d1 + d3, d2);
				minecartentity.setCraftingTableType(item.craftingTableType);
			} else {
				minecartentity = new CraftingTableMinecartEntity(world, d0, d1 + d3, d2);
				minecartentity.setCraftingTableType(CraftingTableType.ACACIA);
			}
	...
	public CraftingTableMinecartItem(Item.Properties builder, CraftingTableType tableType) {
		super(builder);
	...
	public ActionResultType useOn(ItemUseContext context) {
		...
		if (!blockstate.is(BlockTags.RAILS)) {
			...
			if (!world.isClientSide) {
				...
				CraftingTableMinecartEntity minecartentity = new CraftingTableMinecartEntity(world, (double) blockpos.getX() + 0.5D, (double) blockpos.getY() + 0.0625D + d0, (double) blockpos.getZ() + 0.5D);
